...
FSR.site=FSR.sites[FSR.siteid];if(!FSR.site.domain){FSR.site.domain=FSR.getParameter("domain")}if(!FSR.site.name){FSR.site.name=FSR.site.domain
}FSR.sdidx=FSR.getParameter("sdidx");FSR.canceled=false;var C=["files","js_files","image_files","html_files"];
for(var B=0,A=C.length;B<A;B++){if(FSR.site[C[B]]){FSR[C[B]]=FSR.site[C[B]]}}})();FSR.qualified=function(){FSR.controller.process()
};FSR.qualify=function(A){FSR.canceled=false;FSR.qid=A;FSR.c().set("qual",A)};FSR.cancel=function(){FSR.canceled=true
};FSR.QualifierController=new FSR.Class({Implements:FSR.Options,options:{},initialize:function(A){this.setOptions(A);
...
var A=FSR.sdidx;var C=D[A];C.survey=FSR.$merge(FSR.$P().survey,C.survey);C.cancel=FSR.$merge(FSR.$P().survey,C.cancel);
this.sd=C},launch:function(){if(!FSR.canceled){this.popSurvey()}else{this.popCancel()}},popSurvey:function(){var F=FSR.services.survey;
var H=this.sd.survey;var C=(window.screen.width-H.width)/2;var K=(window.screen.height-H.height)/2;
var L=document.location.protocol;var M=F.host;var N=F.path;var I=FSR.c();var B=I.get("rid");var J=this.hash(B);
var G=I.get("current");var D=new FSR.Hash({sid:this.sid(),cid:FSR.id,pattern:G,a:B,b:J,c:24*60*60*1000,version:FSR.version}).toQueryString();
...
this.pop(A,C,K,H.width,H.height)},popCancel:function(){var B=this.sd.cancel;this.page(B);var A=B.url;
FSR.log(500,FSR.c().get("current"));this.pop(A)},pop:function(B,E,D,C,A){var F=arguments.length;(function(){if(F>1){window.moveTo(E,D);
window.resizeTo(C,A)}window.focus();document.location.href=B}).fsr$delay(0)},sid:function(){var F=FSR.c();
var A=this.sd.name;var B=this.sd.site;if(B){A=A+"-"+B}var D=this.sd.section;if(D){A=A+"-"+D}var C=F.get("qual");
if(C){A=A+"-"+C}var E=F.get("locale");if(E){A=A+"-"+E}return A},page:function(E){var B=FSR.c().get("locale");
if(!B){return }var D=E.locale||[];for(var C=0,A=D.length;C<A;C++){if(D[C].locale==B){E.url=D[C].url;
