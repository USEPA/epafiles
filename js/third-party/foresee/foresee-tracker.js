...
};(function(){FSR.siteid=FSR.getParameter("siteid");FSR.site=FSR.sites[FSR.siteid];if(!FSR.site.domain){FSR.site.domain=FSR.getParameter("domain")
}if(!FSR.site.name){FSR.site.name=FSR.site.domain}var C=["files","js_files","image_files","html_files"];
for(var B=0,A=C.length;B<A;B++){if(FSR.site[C[B]]){FSR[C[B]]=FSR.site[C[B]]}}})();FSR.TrackerController=new FSR.Class({Implements:FSR.Options,options:{},R:{START:1,CPPS:2,END:3},initialize:function(A){this.setOptions(A);
this.surveyshown=false;FSR.controller=this},load:function(){fsr$dbug.log("Tracker: load");if(this.running){return this
...
A.run()}})},run:function(){fsr$dbug.log("Tracker: site="+FSR.site.cookie);FSR.c().set("tracker","1");
this.setAlive();this.palive=FSR.Cookie.read("foresee.alive",{path:"/",domain:FSR.site.domain||false});
this.lastseen=FSR.$time();var A=this;this.timer=(function(){A.check()}).fsr$periodical(500);return this
},unload:function(){fsr$dbug.log("Tracker: unload");if(!this.running){return this}this.running=false;
if(!this.surveyshown){FSR.log(201)}FSR.$clear(this.timer);FSR.c().erase("tracker");return this},check:function(){var B=FSR.Cookie.read("foresee.alive",{path:"/",domain:FSR.site.domain||false});
var E=FSR.c();var D=E.get("force")||0;var A=E.get("paused")||0;var C=E.get("timeout");if(B==this.palive){if(A!="1"){fsr$dbug.log("Tracker: Blank for "+((FSR.$time()-this.lastseen)/1000)+" (timeout="+C+")");
...
break;default:this.launch();break}},configure:function(B){var D=FSR.surveydefs;var E=FSR.c();var A=E.get("sd").idx;
var C=D[A];C.idx=A;C.survey=FSR.$merge(FSR.$P().survey,C.survey);C.qualifier=FSR.$merge(FSR.$P().qualifier,C.qualifier);
C.pop=FSR.$merge(FSR.$P().pop,C.pop);this.sd=C},launch:function(){FSR.c().set("survey",1);if(this.sd.pop.what!="qualifier"){this.popSurvey()
}else{this.popQualifier()}},popSurvey:function(){var F=FSR.services.survey;var H=this.sd.survey;var C=(window.screen.width-H.width)/2;
var K=(window.screen.height-H.height)/2;var L=document.location.protocol;var M=F.host;var N=F.path;
var I=FSR.c();var B=I.get("rid");var J=this.hash(B);var G=I.get("current");var D=new FSR.Hash({sid:this.sid(),cid:FSR.id,pattern:G,a:B,b:J,c:24*60*60*1000,version:FSR.version}).toQueryString();
var E=FSR.CPPS.toQueryString();var A=L+"//"+M+N+F.url+"?"+D+"&"+E;if(H.loading){this.page(FSR.$P().loading);
A=(FSR.html_files||FSR.files)+FSR.$P().loading.url+"?url="+A}FSR.log(400,FSR.c().get("current"));
this.pop(A,C,K,H.width,H.height)},popQualifier:function(){var B=this.sd.qualifier;this.page(B);var D=(window.screen.width-B.width)/2;
var C=(window.screen.height-B.height)/2;var A=(FSR.html_files||FSR.files)+(B.url.pop||B.url)+"?siteid="+FSR.siteid+"&sdidx="+this.sd.idx;
FSR.log(300,FSR.c().get("current"));this.pop(A,D,C,B.width,B.height)},pop:function(B,E,D,C,A){(function(){window.moveTo(E,D);
window.resizeTo(C,A);window.focus();document.location.href=B}).fsr$delay(0)},sid:function(){var G=FSR.c();
var A=this.sd.name;var B=this.sd.site;if(B){A=A+"-"+B}var D=this.sd.section;if(D){A=A+"-"+D}var E=this.sd.pop.later;
if(E){A=A+"-"+E}var C=G.get("qual");if(C){A=A+"-"+C}var F=G.get("locale");if(F){A=A+"-"+F}return A
},page:function(E){var B=FSR.c().get("locale");if(!B){return }var D=E.locales||[];for(var C=0,A=D.length;
...
var A=E.get("cpps")||{};var C=E.get("browser");var D={browser:C.name+" "+C.version+" on "+C.platform,url:E.get("current"),pv:E.get("pv"),locale:E.get("locale")||""};
...
},onFailure:function(){}}).send({id:"foresee."+FSR.site.cookie,cpps:FSR.JSON.encode(A)})},setAlive:function(){var A=new Date().getTime();
document.cookie="foresee.alive="+A+";path=/;domain="+FSR.site.domain+";"},hash:function(B){var A=B.split("_");
